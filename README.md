...
The project takes the architectural discipline of  as its baseline, but deliberately extracts only the subset that is useful for a BIM / IFC / document QA product:
...
Those capabilities are valuable in , but they are not required for the first useful `Samolet` release.
...
- `docs/08-microphoenix-adoption-matrix.md` — exact extraction decisions from 
...
```text
samolet/
├── backend/                 # Python backend with extracted  patterns
...
-  extraction dossier
-  adoption matrix and delivery rails
