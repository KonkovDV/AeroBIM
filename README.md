...
Находка несёт пункт нормы, этаж или ось, GUID. Итоговый статус комплекта ставит аттестованный эксперт. На выходе HTML, JSON, PDF и файл BCF.

Три полки стоят в разных местах процесса.
...
Репозиторий работает на **шве между файлами**.

**Как выглядит находка.** Учебный кейс: защитный слой 30 мм в модели, 40 мм на листе — расхождение 10 мм. Сверку считает детерминированный слой. Карточка ведёт к требованию, GUID и строке листа. Принять или отклонить решает человек. Выгрузка — файл BCF 2.1/3.0.
...
| **Спрос** (оператор) | Нулевой вход: веб и файловый обмен. Оплата по подтверждённым находкам — предмет пилота |
| **Техзаказчик** (партнёр) | Минус один круг согласования комплекта. Дельта ревизий: находки → устранено / проигнорировано / новое. HITL |
| **Проектный офис** (партнёр) | Замечание уезжает файлом BCF. Черновик формулировки не ставит итог: `summary.passed` пишет Shared-gate, статус ставит эксперт ([ADR-001](docs/architecture/ADR-001-verdict-ownership-2026.md)) |
| **Информационное моделирование** (партнёр) | Шов документов. Вход — IFC |

Две части защиты. Метрологическая: протокол на учебном комплекте. Прикладная: детерминированный отчёт после канонической ревизии согласованного предмета.
...
| **Пилот закрывает** | Два разметчика-человека + заключение на тот же том (RT-001b) · подписанный профиль назначающей стороны (RT-002c) · system-aware clash (речь **RT-003c**) · федеративный IFC заказчика (`c_customer_federated_ifc`) · импорт BCF в СОД заказчика |
...
Продуктовый Checkpoint — **регуляторно-измерительный MVP**. `customer_go` остаётся **false**. Код и учебные комплекты работают. Публичные линейки и учебные комплекты закрывают измерение там, где пакета назначающей стороны ещё нет. Недифференцированные `closes_rt001/002/003` остаются false.
...
| **RT-003** | `a_federated_geometric_rehearsal` **CLOSED** (**RT-003a**) — посаженный IfcClash (стены; труба против стены). `b_navis_federation_carrier` **CLOSED** — три NWD-федерации. `b_ifc_system_graph_rehearsal` **CLOSED** (**RT-003b**) — граф `IfcSystem` на HVAC-фикстуре (две системы, `IfcRelAssignsToGroup`); не труба против стены | `b_mep_system_clash` **OPEN** (**RT-003c**, `NOT_VERIFIED`) — 0 duct/pipe/cable в IFC заказчика; EIR называет LOD ОВ/ВК/ИТП/ЭОМ/СС, моделей нет. `c_customer_federated_ifc` **OPEN** |
...
Экспорт BCF ZIP — структурный T1 ([`audit/evidence/bcf-structural-handoff-2026-07-25.json`](audit/evidence/bcf-structural-handoff-2026-07-25.json)). Импорт в независимую СОД — **NOT_VERIFIED**. Вход — IFC.

ГОСТ Р 21.101-2026 (приказ Росстандарта № 129-ст от 12 февраля 2026; **в силе с 1 апреля 2026**, взамен 21.101-2020) задаёт GUID как идентификатор электронного документа ПД/РД. AeroBIM ведёт находки к устойчивому идентификатору. Дата введения стандарта (1 апреля) — не дата обязательной подачи ЦИМ АГР в Москве (2 апреля).
...
- Честность доступности проверок; ACL к артефактам на профилях `customer_pilot` / `production` (в development выключено); HTML/JSON; PDF; BCF 2.1 / 3.0
...
- Протокол измерения качества (Уилсон, планировщик выборки; ориентир 0.60)
