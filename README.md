...
В карточке — пункт нормы, этаж или ось, GUID. Эксперт принимает, отклоняет или правит замечание. Флаг комплекта `summary.passed` считает программа. Его не ставит человек и не ставит языковая модель. Наружу уходят HTML, JSON, PDF и BCF.
...
Техлаб Москва, комиссия № 7: автоматическая проверка проектной и рабочей документации. Имя заказчика в открытом репозитории не пишем. Демо-день — 21 сентября 2026. Сами ставим УГТ 4 по ГОСТ Р 58048. Это самооценка, не заключение института.

> Мы дорабатываем контур под заказчика. Одна команда на учебном комплекте показывает находку с доказательствами. На материалах заказчика эффективность ещё не мерили, внедрение не начинали. Checkpoint `GO` — макет для измерения, не акт приёмки. `customer_go` останется false, пока не будет независимого размеченного комплекта, двух людей-разметчиков, подписанного профиля и подтверждённого импорта в СОД.
...
С 2 апреля 2026 года ЦИМ АГР в IFC обязателен к подаче в Москве (постановление Правительства Москвы № 17-ПП от 16 января 2026; совместное распоряжение ДИТ и ДГП № ДГП-Р-1/26/64-16-6/26). С 18 августа 2026 совместное распоряжение ДГП/ДИТ № ДГП-Р-56/26/64-16-473/26 уточняет параметры трёхмерных моделей в информационных системах Москвы — это не формат входа AeroBIM. Это **городские правила подачи**. Вход AeroBIM — IFC.
...
Если `py -3.12` нет, поставьте CPython 3.12 с python.org, не из Microsoft Store. Голый `py` может схватить 3.13. Клонируйте git, не скачивайте ZIP с GitHub. Если IfcOpenShell на Windows не импортируется, поставьте Microsoft VC++ 2015–2022 x64. Проверка окружения — `.\check-launch.bat` из корня клона. Предупреждение показ не останавливает. Код выхода 2 — останавливает. Клон лучше положить в короткий путь латиницей, например `C:\AeroBIM`.
...
tests_passed: backend=3327, frontend=403; commit 52e4c2671bc8; see docs/evidence/runtime-baseline-latest.json · src ~107329 LOC; tests ~70151 LOC; extraction macro_f1=0.8600000000000001 (fixture corpus; not product accuracy)
