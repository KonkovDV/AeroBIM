...
`.[vision]` включён в базовый контур разработки, а `.[clash]` и `.[docling]` остаются опциональными extras. Это технически допустимо, но должно быть явно отражено в эксплуатационной документации и в описании границ возможностей (capability framing).
