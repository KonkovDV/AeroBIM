...
Демо-день 21.09.2026. Дека: [`AeroBIM.pptx`](../submission/03-presentation/AeroBIM.pptx) · [`AeroBIM.pdf`](../submission/03-presentation/AeroBIM.pdf) · [текст](../submission/03-presentation/demo_day_slides.md). Живая команда — [Try it](../README.md#try-it): с корня клона `run-jury.bat` / `./run-jury.sh`, либо `python -m aerobim.tools.run_kt3_jury` из `backend/` после установки `".[dev,raster]"`.
...
| [Текст слайдов](../submission/03-presentation/demo_day_slides.md) | Текстовая копия показа: 7 слайдов |
| [PPTX демо-дня](../submission/03-presentation/AeroBIM.pptx) | Поле формы; 7 слайдов; PDF — [`AeroBIM.pdf`](../submission/03-presentation/AeroBIM.pdf) |
