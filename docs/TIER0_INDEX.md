...
| PDF демо-дня (`AeroBIM_demo_day.pdf`) | Поле формы; бинарь **не в git** (kitchen B-01) |
...
Пять полей формы: [пакет подачи](../submission/README.md). Канон речи в git: [`demo_day_slides.md`](../submission/03-presentation/demo_day_slides.md). Бинарный `AeroBIM_demo_day.pdf` не отслеживается (kitchen B-01). Ролик 2–3 мин **не записываем и не прилагаем.** Показ — `run_demo_ifc_acceptance_gate`. Демо-день — `run_kt3_jury`.
