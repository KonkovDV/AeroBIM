...
CI pin: `commit_sha` / `tests_passed` / `tests_collected` в [`../evidence/runtime-baseline-latest.json`](../evidence/runtime-baseline-latest.json) (`attested_by=ci`). Строка «1980 проверок / 656 тестов» ниже — **срез 14.08**, не текущий pin. Локальный pytest не подменяет CI.
