...
Frontend (`npm ci` / `npm run dev`) — не показ КТ#3, если время жюри <12 мин. Оболочка ревью: кнопка «Загрузить демонстрационный комплект» (`POST /v1/demo/seed-fixture`, только development). Не говорить «рабочее место сдано». Не сидировать в грязный `var/reports` как «объём канала».
