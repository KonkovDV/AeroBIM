...
### Критическое издание ответов 25.08 (разбор 28.08) `[НОВОЕ]`

Работа велась по пересказу в чате; авторитетен сам документ ответов. Двенадцать
расхождений пересказа с документом внутренне зафиксированы; в git — только
безымянные следствия. Ключевые: источник сверки — **расчётные записки
PDF/Excel**, а не бинарные файлы комплекса; добавлены объекты сверки **нагрузки
и площади**; сводная модель существует **в NWD**; соответствие внутренним
стандартам **обязательно**, их перечень выдан 25.08 двумя ссылками в ответе
1.2.1 (внутренние пути контура заказчика; нужна публикация тем же способом,
что датасет); прямая интеграция с СОД на MVP **не требуется** (п. 2.2.2);
числовые критерии (>90% коллизии, время) в ответах **не упомянуты** — не
подтверждены, не уточнены, не сняты.

**ТР-67.** Сверка объёмов спецификации с графикой/BIM — у заказчика названа
«логическими коллизиями» (п. 2.1.3). Это **не** «объёмы из модели в смету»
(этот контур у заказчика уже закрыт BIM-платформой данных). Обязательное
требование; статус **partial** (объявленные тройки в `compare_spec_volumes`,
без ingest комплекта и без корпуса заказчика).  
**ТР-68.** Замечание цитирует пункт **нормы или СТО** (п. 2.1.5), не только СП.
Терминология по СП 63: «класс бетона/стали», не «марка».

Остаток критического издания (не отдельные ТР, но тормоза речи):
- п. 1.2.3 — база типовых узлов в тех же двух папках, форматы только PDF/DWG,
  узлов в IFC нет; проверка узлов идёт через 2D-контур (частичное покрытие),
  DWG не читается (RT-TYP-NODES);
- п. 3.1.1 — облако допустимо, развёртывание в контуре заказчика не требуется;
  требуется изоляция **по проектам** (модель доступа, не шифрование)
  (RT-CLOUD-ISO);
- п. 3.1.2 — заказчик сам записал «обезличенные комплекты в рамках NDA»;
  фактическая передача этому не соответствует — запрос к организаторам
  исполнить этот пункт (RT-NDA-STATED);
- п. 3.2.2 — горизонтальное масштабирование заложить архитектурно, не
  реализовывать на MVP; на защите — точки расширения, не нагрузочные цифры
  (RT-SCALE-MVP).
...
| ТР-67 | `compare_spec_volumes` | **partial** | P1 | Объявленные тройки спецификация↔график↔BIM | Ответы п. 2.1.3 |
| ТР-68 | Remark citation | partial | P1 | Ссылка на пункт нормы/**СТО** | Ответы п. 2.1.5 |
