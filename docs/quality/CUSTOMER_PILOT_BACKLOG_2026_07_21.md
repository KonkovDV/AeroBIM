...
| Bucket | Items |
|---|---|
| **Works (fixture-proven)** | Package analyze; IFC/IDS/cross-doc; `PackageOutcome`; intake fail-closed; provenance; BCF ZIP T1 + MEP topics; HITL; ACL 404; SSRF; jobs; precision/SLA gates (eng); revision merge; run manifest; golden hash; stage timeout; **IFC parse cache + spatial index**; **annotation↔IFC claimed-GUID presence confirm**; **core PDF pypdfium2/pdfminer (LIC-001 Option B)**; **federated IfcSystem graph + clearance matrix eval (fixture)**; Docker offline image-track |
| **Experimental** | OpenCDE BCF API; IfcSystemAwareClash probe + AABB broadphase (`geometry_verified=False`); IFC KG advisory; compliance agent tool traces |
...
| RT-P2-007 | Invented IFC guids | `ifc_guid` only after claimed GUID + spatial-index presence; else `claimed_guid:` evidence |
...
| P2-02 | Federated graph + matrix + BCF honesty | **ENG_PARTIAL** (edge_kinds + AABB broadphase; geometry NOT_VERIFIED; RT-003 OPEN) |
...
| P2-02 | Federated IfcSystem graph + clearance matrix + BCF honesty | **ENG_PARTIAL** (edge provenance + AABB filter + analyze integration; geometry still NOT_VERIFIED) |
