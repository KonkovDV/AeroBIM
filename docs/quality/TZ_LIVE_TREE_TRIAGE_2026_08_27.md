...
PR-diff к `main` на первом проходе 27.08 был пустой. Этот файл — полный триаж живого дерева плюс проходы КТ#3 / OOS / локация замечания / канал 25.08 / таблица LIRA / стриминг IFC / HTTP `.lir` / JSON sidecar индекса / чек-листы типовых замечаний и счётчики страницы MVP (28.08) / адрес СОД заказчика (28.08) / неизмеримость критерия коллизий на поставленном пакете (28.08) / критическое издание ответов 25.08: выданный перечень стандартов, NWD-федерация, записки вместо бинарей, 2.2.2, молчание по 90%/времени, сверка объёмов, типовые узлы PDF/DWG, облако с изоляцией по проектам, масштаб как архитектура, п. 3.1.2 как условие заказчика (28.08). Не закрытие RT.
...
| RT-SPEC-VOL | «Логические коллизии» = геометрия / пропустить их | П. 2.1.3: сверка объёмов спецификации с графикой/BIM; ТР-67, строка 31 матрицы — MISSING |
