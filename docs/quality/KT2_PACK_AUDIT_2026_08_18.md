...
|---|---|---|
| Быстрые гейты CI | `scripts/pre_push_gate.py` (ruff, claims ×4, baseline, metadata, links, S-band, handoff) | ok; одно предупреждение об отставании pin на несколько коммитов — в CI невидимо |
...
| CI / CodeQL на `main` | последний прогон на `386dba8` | success / success |
| Дерево | `git status` | чистое; локальные материалы не отслеживаются git |

## Что было найдено и закрыто вечером 17.08

| Находка | Реакция |
|---|---|
| Дека не была отслежена в git | pptx/pdf закоммичены; поля 01/02/05 и карта ТЗ указывают на деку; тест `test_submission_surfaces_are_consistent_about_deck_and_video` фиксирует |
| README тащил скриншот фикстуры как витрину | Блок удалён из обоих README; доказательство осталось в evidence-пакете |
| Дрейф baseline тестов (LOC 51404 → 51459) | Обновление из артефакта прогона CI, не из локального pytest |
...
1. ЛК: пять полей, файлы собраны локально вне git (видео-поле пустое — показ живой CLI).
