...
**Объект:** публичный `main` после [`f9389bf`](https://github.com/KonkovDV/AeroBIM/commit/f9389bf).  
...
Человеческие условия GO (юрлицо, LOI, пакет + два разметчика, именная СОД, видео оператора) кодом не создаются. Сочинить их — consequential-validity failure.
