...
| UI expert workplace (01.09) | `ui_expert_workplace_triage_snapshot`; review shell + git seed; TZ Web UI **partial**; jury track CLI |
| SPG Aug 2026 speech pin | `spg_speech_triage_snapshot`; construction ≠ FM; PDF not in git; not a jury exhibit |
...
| — | Речь про RT-002 | Всегда split: 002a CLOSED (публичные IDS Мособлгосэкспертизы/АГР/СПб ЦГЭ + `pack_hash`), 002b CLOSED (EIR v4 как текст), 002c OPEN (нет подписи Самолёта) | «RT-002 открыт целиком» = «нет норм»; «RT-002 CLOSED» без split |
