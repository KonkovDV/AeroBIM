...
1. **Публикация двух папок из ответа 1.2.1** (стандарты компании; регламенты BIM) тем же способом, что датасет, либо одним архивом; для старта достаточно двух документов (СТО по КЖ/КР + регламент BIM). Перечень **выдан** — просим переключатель доступа, не подборку. Каждое правило вернём на подтверждение с точной цитатой.
...
5. Исполнение п. 3.1.2 самих ответов: обезличенные комплекты в рамках NDA — что делать с файлами, которые обезличенными не являются.
