...
| C-01 | CI backend tests passed / collected / failed / skipped | 3399 / 3420 / 0 / 21 | docs/evidence/runtime-baseline-latest.json backend.*; attestation.attested_by=ci | VERIFIED_INTERNAL | да | CI: бэкенд 3399 из 3420, 0 упало, 21 skipped. |
| C-02 | CI frontend tests passed / failed | 403 / 0 | docs/evidence/runtime-baseline-latest.json frontend | VERIFIED_INTERNAL | да | CI: фронт 403 / 0. Только из runtime-baseline. |
...
| C-05 | Architecture inventory | 48 protocols / 77 adapters / 63 DI tokens | docs/evidence/runtime-baseline-latest.json architecture_inventory | VERIFIED_INTERNAL | нет | инвентарь архитектуры не критерий жюри |
...
37. Закрытый CAD Autodesk и `.lir` не есть продукт приёма. Обмен КТ#3 — IFC и PDF/A. Штатный Navisworks IFC не пишет. Пробный ODA — замер, не продукт ([`quality/FORMAT_INGEST_TRIAGE_2026_09.md`](FORMAT_INGEST_TRIAGE_2026_09.md)).
...
39. Браузер — оболочка ревью, не сданный полный цикл рабочего места эксперта. Оболочка не пишет `summary.passed`. Native RVT, NWD и DWG остаются в закрытом отказе. 30 минут и 5–10 комплектов в день — цели ТЗ, не измеренный SLA. Живого коннектора к 10D и Tangl нет ([`quality/UI_EXPERT_WORKPLACE_TRIAGE_2026_09.md`](UI_EXPERT_WORKPLACE_TRIAGE_2026_09.md)).
