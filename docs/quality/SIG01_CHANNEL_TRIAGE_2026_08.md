...
Носители (без имён): 15 IFC, схема IFC2X3; 6 АР + 5 КР в пакете A; QTO `NetFloorArea` = 0; заполненный `FireRating` стен пакета A — класс **EI 45**, не учебный `REI60` ([deep-study](../evidence/deep-study-carrier-facts-2026-08.md), IUA `CH-09`). PDF-выборка даёт HITL, не находки чертежа.
...
| RT-SIG01-DEFECT | Объём канала = дефекты заказчика канала | `is_customer_defect_list=false`; только фраза отчёта |
| RT-SIG01-PACK | 15/15 IFC прогнаны = пакет обработан | `is_pack_processed=false`; census `processed=false` |
| RT-SIG01-SP | Unsigned ALL+eq REI60 = проверка СП 2.13130 / СП 63 | Учебные `customer-*.txt` / TYP-AR; RT-002c OPEN |
| RT-SIG01-EI45 | EI 45 vs демо-REI60 = провал огнестойкости | IUA CH-09; не класс II / C0 |
...
| RT-SIG01-OVERLAP | REQ-FIRE-001 + TYP-AR-011 = два дефекта | `unsigned_rule_overlap`; exists+eq на одном ключе |
| RT-SIG01-KR-DOOR | «Нет IfcDoor» на файле КР = нет дверей как дефект | `entity_presence`; 6 АР + 5 КР |
| RT-SIG01-PDF-HITL | Строки HITL PDF = находки чертежа / счёт дверей | `service_hitl`; IUA CH-03 |
...
| RT-SIG01-SLA | Время RocksDB = SLA заказчика | `publishable_sla=false`; IUA CH-06 |
...
| RT-SIG01-RAIL | TYP-AR-020 ≥ 1,2 м = проверка перил по СП | Демо-порог шаблона; `unrestricted_eq_sample` |
...
2. **Overlap.** Пара `REQ-*` (eq) и `TYP-AR-*` (exists) на том же `entity+pset+prop` — один факт покрытия, не два дефекта. Карта: `unsigned_rule_overlap.overlap_snapshot()`.
