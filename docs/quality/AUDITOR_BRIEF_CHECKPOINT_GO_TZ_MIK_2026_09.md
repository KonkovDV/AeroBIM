...
| КТ#3 | 03–21.09.2026 | feature freeze 18.09; delivery 19–21.09; демо-день 21.09 |
...
31. «Фронт сдан» / матрица ТЗ «Web UI done» / Vite как показ на чужом ноутбуке жюри / «две находки огнестойкости = точность». Review shell — не показ сидящего члена жюри. Показ жюри = CLI. Seed — git-фикстура.
...
Пункт 2 приказа разводит **две** таблицы. Июньский порядок считает **обе**
окружности **средним арифметическим**. Баллы закреплены за критериями, не за
креслами. PDF приказа Фонда **в git нет**. Веса — IUA над копиями владельца
07.09.2026 приказа **П-01-ОД-52-1/26** от **17.06.2026**. Не `attested_by=ci`.
`predicted_aerobim_total() is None`.

| Слой | Отбор (прил. 2) | Финал (прил. 3) |
|---|---|---|
| Где критерии | Приложение **2** к Положению | Приложение **3** к Положению |
| Как в этом корпусе | К1–К5 | Б1–Б5 переписаны с копий; PDF **не видели** в git (`regulation_appendix_3_in_git = false`) |
| Как считают | **среднее арифметическое** оценок членов | то же среднее, не историческая **итоговая сумма баллов** |
| Кто | комиссия №7 (пять кресел) | те же пять; присутствие — протокол |
| Зачем | допуск | победа и приз |
| Тай-брейк | система A: **К3, затем К4** (К2 не участвует) | только **Б1** (соответствие задаче партнёра) |
| Кворум | ≥ половины состава и **≥3** | при трёх голосах вес каждого = 1/3 среднего |
| Шире пяти кресел | нет (номинал 5) | нет (`FINAL_ROUND_WIDER_THAN_NOMINAL = false`) |

Порог приза: не менее **50** из 100. В приложении 3 п. 1.4 встречается «менее 50». Рабочий порог **50**; неоднозначность — аргумент на границе, не лицензия округлить вверх. `prize_floor_denominator_known = true`. Порог не гарантирует победу, договор или выплату.
...
Номинальная атрибуция кресел **опровергнута** копиями: партнёрские три кресла не владеют К1+К3+К5=65; кресла Фонда не владеют К2+К4=35. Баллы за критериями. Три кресла партнёра внесены **по согласованию** — участие не гарантировано приказом. Два кресла Фонда — штатные. Командные 40 баллов К1 — отбор, не финал.
...
### 9.4 Система B (Приложение 3 по копиям; PDF не в git), среднее
...
`finalist_weights_are_regulation_appendix_3 = true`. Pytest и fixture SLA **не** поднимают Б2 в «высокий». Система B не берёт порог 50 при отсутствии метрик партнёра как «высокий Б2». Готовность к интеграции на MVP — файловый обмен, не обязательный прямой API.
...
E. MIK
   ...
   - Система A и система B — среднее; историческая «сумма» опровергнута
   - Приложение 3 PDF не в git (перенос с копий)
   - Три кресла не владеют 65
   - 52.6 identity ≠ score
   - УГТ 4 ≠ 5
