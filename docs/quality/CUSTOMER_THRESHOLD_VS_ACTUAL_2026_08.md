...
claim_boundary: >
  ...
  the appointing party ingest. Checkpoint GO; customer_go false.
---

# Целевое против фактического (вторая система)

Это **не** Приложение 3 к Положению (PDF в git мы не видели; веса Б1–Б5
переписаны отдельно). Это технические пороги карточки и ТЗ, по которым
кресла партнёра будут ставить свои баллы в финале.
...
| Предельные размеры | 500 МБ документы, 1,5 ГБ модели | SPF / WASM — **256 МиБ**. До 1,5 ГБ — RocksDB под `customer_pilot`. RSS на 1,5 ГБ не замерен. **Не поднимаем** SPF default |
...
1. Штатный пакетный экспорт сводной модели NWD→IFC — блокирующий предмет
   измерения коллизий. Имена площадок и папок в git **не** пишем.
2. Уже выданные стандарты/регламенты — тем же каналом, что датасет.
   Локаторы внутренних папок в git **не** пишем.
