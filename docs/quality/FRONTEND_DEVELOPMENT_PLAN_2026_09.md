...
| 09.09 | Прогон пайплайна на стенде | 23 шаги 0–3 сняты (HTML/JSON/BCF/PDF в smoke); 25 шаг 0 в smoke; 26 partial (своя машина); demo-seed — отдельный трек; §8.0.2–8.0.3 |
...
на уже подтверждённую находку. `run_live_review_smoke` гоняет три захвата
(список → чертёж → карточка → решение; затем call-path demo-seed) и ставит
`AEROBIM_PRIORITY_PROFILE=samolet`. Таймаут блокировки HITL после проигранной
гонки — конфликт состояния, не `RuntimeError`. `customer_go` false.
...
Стек помечен `vite-dev`: это трек ИТ-ментора, не `python -m aerobim.tools.run_kt3_jury`.

### 8.0.3. Жюри / Red Team: швы, которые 8.0.2 не закрывал (09.09)

Триаж МИК × Самолёт по живому контуру. Не закрывает OA-21 владельца и не
заменяет чужой офлайн-ноутбук (WP-FE-26). `customer_go` false.

| Шов | Что сделано в прогоне | Чего нет |
|---|---|---|
| OA-21 только JSON | `smoke:decision` снимает HTML / JSON / BCF 2.1 / PDF до и после подтверждения; черновик не содержит сохранённую правку; confirmed — `state=accepted` и тот же текст; «исправлено» запрещено. PDF — ненулевой файл + utf8-скан байтов (не pypdfium) | Строка владельца OA-21 |
| Кнопка колла 15.09 | `smoke:demo` жмёт «Загрузить демонстрационный комплект» и **ждёт POST** `/v1/demo/seed-fixture`; маршрут гоняет **AnalyzeProjectPackage** по git-пакету `samples/demo/vertical-slice-2026-08-11` (IFC + IDS + ТЗ + лист A-101), не голый `validate`. `report_id` не должен совпасть с overlay-фикстурой `seed_smoke_report`. Оверлей на этом отчёте не требуется. `useReports` не автовыбирает overlay, если есть другой отчёт, и не держит overlay, пока POST сида в полёте | Production-сборка кнопку скрывает (`import.meta.env.DEV`) |
| Честность на экране | Баннер учебного набора, шлюз машина/HITL, PDF как черновик покрытия, роль ≠ RBAC, XLSX нет, pass-badge на фикстуре запрещён | Литерал `customer_go` в шапке (убран из primary copy) |
| 3D | Ждём `.viewer-status-ready`, а не только текст «пара клэша» | Геометрическая приёмка модели |
| Тёмная ОС (WP-FE-25 шаг 0) | Playwright `colorScheme: dark`; `:root` остаётся `color-scheme: light` | Брендбук / переключатель темы |
| Клавиши J/K/E/? | Справка открывается, E фокусирует редактор. A/R не жмём в этом шаге — решение идёт кнопками | Раскладка RU на чужой машине |
| `vite preview` | **Не** в этой цепочке. Production CSP `connect-src 'self'`, у preview нет `/v1` proxy, кнопка демо пропадает | Отдельный IT-стенд с reverse-proxy, если понадобится |

`--skip-demo-seed` оставляет только overlay-фикстуру. По умолчанию demo-seed включён:
это маршрут повестки 15.09, его нельзя подменять сидом с оверлеем.
...
**Статус: шаги 0–3 в автоматизированном прогоне 09.09 (§8.0.3).** UI предупреждает о несохранённом черновике (`window.confirm`, точная копия). JSON/HTML/PDF несут `issue.review` с сервера (`include_review=True`); BCF — `review_events`. Живая пара HTML/JSON/BCF/PDF снимается `smoke:decision`. Строку владельца OA-21 закрывает владелец.
...
**Статус: done, 09.09.** Оболочка фиксирована светлой: `color-scheme: light` + `force-light.css`. Тёмные ветки токенов в `design-system.css` / `01-tokens.css` и компонентах сняты, чтобы OS dark не мог подмешать палитру, если замок не последний. Не брендбук и не переключатель темы.
...
- Шаг 0: прогнать `npm run smoke:browser` с эмуляцией `prefers-color-scheme: dark`
  (в `run_live_review_smoke` контекст Playwright — `colorScheme: dark`)
...
**Статус: partial, 09.09.** axe в jsdom: шапка, навигация, баннер, экспорт, загрузка, прогон, эксперт (пустой), сравнение версий, «Эффект», индекс проектов, чертёж (заглушка), заглушка вьюера, цикл комплекта. Контраст светлых токенов — `token-contrast.test.ts`. Правило color-contrast в axe выключено. Это не сертификат WCAG.

Сейчас `src/test/a11y-smoke.test.tsx` покрывает список, карточку, оболочку и восемь экранов ТЗ заглушками (пустой эксперт, прогон, diff, «Эффект», проекты, чертёж, цикл). Правило контраста в axe выключено (строки `color-contrast: { enabled: false }`).
...
### Волна D. Внешний обзор D-04…D-12 (после PR #47)

Не закрывает RT-001/002/003. Не «фронт сдан».

#### WP-FE-34. Ориентир `<main>` (P1, D-04)

**Статус: code, 10.09.** Один `<main id="work-area">` в `App.tsx`. Сетка эксперта и индекс проектов — `div`. Skip ведёт в landmark, не в пустой 0-height sink. Это не сертификат WCAG.

#### WP-FE-35. Виртуализация списка (P1, D-05)

**Статус: partial.** `VIRTUALIZE_AFTER = 40` уже в `FindingListPanel`. Не замерено на 1508 находках заказчика. Не объявлять закрытым.

#### WP-FE-36. Липкие заголовки таблиц (P2, D-06)

**Статус: partial.** `position: sticky` у `.coverage-table th` и `.capability-table th`. Не закрывать без длинного живого списка.

#### WP-FE-37. Слои отчёта и «не проверялось» (P1, D-07/D-08)

**Статус: partial, 10.09.** D-08: карта покрытия на экране «Эффект» **первая**. D-07 (решение → находка → доказательство) **открыт**.
