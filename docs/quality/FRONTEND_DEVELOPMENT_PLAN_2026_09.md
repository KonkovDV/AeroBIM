...
из dev-CSP убран. `frame-ancestors` в `<meta>` браузер игнорирует по
спецификации; реальная защита — заголовок бэкенда `X-Frame-Options: DENY`
(`SECURITY.md`), меняться она от этого не должна.
