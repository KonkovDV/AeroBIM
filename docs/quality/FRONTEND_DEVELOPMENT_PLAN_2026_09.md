...
| Кнопка колла 15.09 | `smoke:demo` жмёт «Загрузить демонстрационный комплект» и **ждёт POST** `/v1/demo/seed-fixture`; `report_id` не должен совпасть с overlay-фикстурой `seed_smoke_report`. Оверлей на этом отчёте не требуется | Production-сборка кнопку скрывает (`import.meta.env.DEV`); `useReports` автовыбирает первый отчёт — без ожидания POST прогон садится на overlay |
