...
При этом проект пока не является полноценной платформой координации BIM-ревью. Остаются ограничения (май 2026):

- развитая эргономика 2D overlay (базовый контракт и панель есть; нужна полировка UX);
- навигация по парам коллизий и множественному выделению;
...
`.[vision]` включён в базовый контур разработки, а `.[clash]` и `.[docling]` остаются опциональными extras. Это технически допустимо, но границы возможностей должны быть явно описаны в эксплуатационной документации и в [`pilot-claim-boundary-2026.md`](../pilot-claim-boundary-2026.md).
...
Это хороший индикатор зрелости репозитория: проект уже перешёл в фазу системного укрепления, а не только обсуждения дорожной карты.

## 7. Приоритетные рекомендации

### R1. Сделать review-слой пространственно действенным

Следующий этап — навигация по замечаниям и устойчивость обзора:

1. Полировка 2D overlay (зоны проблем на чертежах).
2. Улучшенная навигация по issue/clash поверх `web-ifc + Three.js`.
3. Сквозной smoke одного отчёта (полный UI flow на сохранённом отчёте) — уже есть в CI; расширить на пилотные пакеты.
4. Интеграционные проверки IFC-источника в scope отчёта.
...
Любое усиление извлечения (включая стохастический анализ растровых чертежей) должно идти за существующими портами и сохранять `DrawingAnnotation`/`ProblemZone` как продуктовую истину.
