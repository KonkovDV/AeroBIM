...
|---|---|---|---|
| P1 | **DWG conversion MVP** (вариант A + opt-in B из [gap-анализа](../pilot/FOUR_DIRECTION_GAP_ANALYSIS_2026_07_24.md) §1.3): ingest `.dwg` как артефакт + hash, регистрация `source_dwg_sha256↔derived_*`, loss report, fail-closed при отсутствии derived. **Шаги 3–6 закрыты 2026-07-27** ([Wave S](../quality/DWG_DERIVED_PROVENANCE_WAVE_2026_07_27.md)): hash-верификация sidecar + conversion-loss QA (пороги §1.4) + analyze-интеграция + CLI `aerobim-register-dwg-conversion`; осталось: согласованные инвентари листов/слоёв и конвертер от Самолёта | Только если Самолёт даёт DWG + согласованный конвертер; иначе фиксируется как ограничение | `dwg_dxf=ok` запрещён honesty-гейтом |
