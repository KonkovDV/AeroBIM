...
- AeroBIM: pins + gitignored checkout of QA CSV + non-GPL IFC (`duplex`, `dental_clinic`, `digital_hub`, `sixty5`, `wbdg_office`, `west_riverside_hospital`). GPLv3 dirs never enter git. Deterministic smoke **27/1026** scored (`output_sha256=0a1679f8…9bfa35`). Of those 27, eval-split has **12** test / **15** train — not a 514 false-pass figure. `skip_breakdown` (gpl 189 / incomplete 110 / non_numeric 66 / unmapped_nl 634) is the honest denominator, not extra accuracy. `west_riverside` is on disk for federated MEP inventory; it has **0** rows in the v2 QA CSV. Smoke: `docs/evidence/ifc-bench-v2-smoke-latest.json`
