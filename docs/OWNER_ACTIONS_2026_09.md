...
| OA-8 | Пакет вопросов заказчику: **шесть блокеров ТЗ** (точность >90%/типовые ошибки, комплект 30 мин + ревизия ПД/РД, записки расчётов, сети IFC, площади, подпись норм); NDA и МИК-формы **не** в этом письме. Актуализация 03.09: книги замечаний ЦНЭ по эталону найдены в архиве — вопрос 2 включает подтверждение канонической редакции gold labels (письмо v2.2.0 в `.local/`) | К3; столпы ТЗ | отправить **03.09**, ответ к **08.09** | Исходящее письмо с датой требуемого ответа; без ответа вопрос уходит из скоупа КТ#3 |
