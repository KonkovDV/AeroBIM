...
| OA-8 | Пакет вопросов заказчику одним документом через организаторов: правовой блок (6 вопросов от 28.08), эталонный объект и ревизии, профиль приёмки (IDS), два разметчика по 8 ч, выгрузка IFC из Revit-контура, примеры расчётных записок, BCF-импорт в СОД; TBD-разделы ТЗ подаём как **нашу редакцию v2 (`docs/tz/`) на подтверждение**, не как просьбу «заполнить» | К3, Б1, Б2; задачи 1, 2, 4, 6, 7 трекера | отправить **31.08**, ответ к **08.09** | Исходящее письмо с датой требуемого ответа; без ответа вопрос уходит из скоупа КТ#3 |
...
| OA-12 | Публикация реестра пакета заказчика (задача 2 трекера) в трек-чат | задача 2 трекера; доказательная таблица для юрслужбы по OA-9 | **02.09** | Реестр (путь/имя/расширение/размер/MIME/SHA-256/обрабатывается/приоритет/правовой флаг) опубликован в чате; артефакт собран локально, публикует владелец |
...
| OA-14 | Решение по «неэффективному использованию пространства» (заказчик назвал 25.08: норматив продаваемой площади, МОП, коридоры): в скоуп КТ#3 как advisory по внутреннему нормативу, или явная позиция «вне MVP» с формулировкой для жюри | задача трекера 1/4; К3; карта покрытия ТЗ | до репетиции защиты **22.09** | Позиция записана в карте покрытия и карточке речи; ни «реализовано», ни «не нужно» не произносится |
