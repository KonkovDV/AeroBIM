...
| GPT-5.2 | 0,49 |
| **AeroBIM live Qwen 3.6-35b (Yandex AI Studio)** | **0,4325** |
| Claude Opus 4.5 | 0,42 |
| GLM-4.6V (лучшая открытая в paper) | 0,39 |

Скорер сходится с Table 1 авторов в допуске \|Δ\|≤0,02.  
**Формулировка для чата:** открытая модель через российское облако — в одном порядке с фронтиром на *этой* задаче. Не «обошли Gemini». Не точность нормоконтроля.
