...
claim_boundary: >
  ...
  delivered. Not a customer signature. Checkpoint GO; customer_go false.
---

# Блокеры RT: чем заменили заказчика канала, что осталось
...
| **RT-002** | `a_regulatory` | **CLOSED** | Публичные IDS Мособлгосэкспертизы (**24** `.ids`), СПб ГАУ ЦГЭ (**22** `.ids`, `signed_by_customer=false`), ЦИМ АГР Москвы (**4** `.ids` + pack `moscow_agr_2026`, city as publisher). Линейка измерения, не EIR заказчика канала. Машинный порог: ≥20 MOEXP + ≥15 CGE + ≥3 AGR |
...
| **RT-002** | `c_corporate_signed` (`b_corporate`) | **OPEN** | Подпись заказчика канала на профиле приёмки / `customer_approved`. Текст EIR и город-издатель ≠ подпись |
...
Речь: измерение ведём на публичных IDS, тексте EIR/стандарта с канала и учебных комплектах; подпись заказчика канала, граф ИОС в IFC и публикуемая точность — отдельные тома.
