...
| **Qwen local (KT#2 W1)** | Feasibility + plan; `private_qwen_local` / `private_yandex_ai_studio`; OpenAI-compat + host allowlist + token caps + `model_revision` boot pin; compose CLI; OFF==ON (incl. named `test_advisory_vlm_off_equals_on`); Alibaba Max NOT_VERIFIED; `tier_defaults.private` = local weights; stamp `layout_role` exclude before cloud VLM | Studio cloud PUBLIC/INTERNAL only (never private default); not verdict; grant ≠ RT-001 bottleneck |
