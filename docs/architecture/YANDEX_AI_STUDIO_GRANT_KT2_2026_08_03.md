...
1. **Budget caps (fail-closed):** max tokens per call / per run / per day; charge estimate on transport failure and on every 429 retry; optional `AEROBIM_LLM_BUDGET_LEDGER` for shared day counters across workers (without it: `budget_scope=process_local` — N workers ≈ N× daily cap); `AEROBIM_LLM_BUDGET_TZ` for day-roll (default UTC). Stale `.lock` is cleared by mtime; if the lock still cannot be acquired, `lock_degraded=true` / `budget_scope=file_shared_lock_degraded` appears in usage (RT-LEDGER-01 — never silent). **TOCTOU bound (RT-LEDGER-02):** `check_before` and `record` are separate lock acquisitions; on the day-limit edge, N workers may each pass then charge → overshoot ≤ `N × max_tokens_per_call` (e.g. 8×4096 ≈ 32k tokens ≈ ~16 ₽). Accepted without reservation; do not treat ledger as a hard distributed gate.
