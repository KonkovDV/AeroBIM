...
`AeroBIM` does not clone . It extracts the subset of  that creates durable architectural leverage for a BIM QA product.
...
|  Surface | Extract Into AeroBIM | Why |
|---|---|---|
...
## Decision 1: Python-First Backend
 ...
 is TypeScript-first, but `AeroBIM` should be Python-first because the target validation ecosystem is strongest there.
