...
`Samolet` does not clone . It extracts the subset of  that creates durable architectural leverage for a BIM QA product.
...
- `adopt` when the  pattern can transfer directly with minimal adaptation;
...
|  Surface | Extract Into Samolet | Why |
|---|---|---|
...
### Anti-Stub Discipline
 ...
 treats fake infrastructure as a source of architectural drift.
...
### Verification Rails
 ...
 uses explicit closure rails instead of informal "looks done" decisions.
...
### Search Before Building
 ...
 donor extraction is explicit about not inventing new abstractions before checking what already exists.
...
|  Capability | Why It Is Excluded From MVP |
|---|---|
...
## Decision 1: Python-First Backend
 ...
 is TypeScript-first, but `Samolet` should be Python-first because the target validation ecosystem is strongest there.

This is not a rejection of . It is a faithful translation of the same architectural shape into a better-suited runtime.

## Decision 2: One Narrow MVP Use Case

 contains many subsystems because it is a platform.
...
That is the smallest subset that preserves the  discipline while producing product value.

## Where The Next Pass Lives

For exact keep/adapt/defer decisions, see `08-microphoenix-adoption-matrix.md`.

For the operational delivery rules derived from , see `09-implementation-and-verification-rails.md`.
