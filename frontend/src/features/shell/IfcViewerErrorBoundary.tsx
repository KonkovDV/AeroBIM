 ...
 * бросает синхронную ошибку в render — React без границы размонтирует всё
 * дерево приложения. Граница изолирует сбой: список замечаний, карточка и
 * экспорт продолжают работать.
 *
 * Не заменяет обработку ошибок в самом `IfcViewerPanel` (async init / load):
 * там ошибки ловятся в `useEffect` и показываются через `viewerError`.
 */
