...
function encodeFinalRemarkNote(text: string): string {
  return `${FINAL_REMARK_NOTE_PREFIX}${text}`;
}

function decodeFinalRemarkNote(note: string | null | undefined): string | null {
  if (typeof note !== "string" || !note.startsWith(FINAL_REMARK_NOTE_PREFIX)) {
    return null;
  }
  const suffix = note.slice(FINAL_REMARK_NOTE_PREFIX.length);
...
function effectiveRemarkText(
  ...
  let latest = baseEffectiveRemarkText(issue, events) || issue.message || "";
  for (const event of events) {
    if (!eventMatchesIssue(event, issue)) {
      continue;
    }
    if (event.event_type === "accepted" || event.event_type === "rejected") {
      const finalText = decodeFinalRemarkNote(event.note);
      if (finalText !== null) {
        latest = finalText;
      }
    }
...
export type SelectedReportState = {
  ...
  hitlRequestState: HitlRequestState;
  ...
  openRemark: (issue: ValidationIssue | null) => Promise<void>;
...
function resetEditor(
  ...
  setRemarkDraft: Dispatch<SetStateAction<string>>,
  setRemarkSaveState: Dispatch<SetStateAction<RemarkSaveState>>,
  setHitlRequestState: Dispatch<SetStateAction<HitlRequestState>>,
...
export function useSelectedReport(
  ...
  const [hitlRequestState, setHitlRequestState] = useState<HitlRequestState>("idle");
  ...
  useEffect(() => {
    ...
    fetchReport(selectedReportId, { signal: controller.signal })
      .then(async (report) => {
        ...
        const initialDraft = landingIssue ? effectiveRemarkText(landingIssue, []) : "";
        remarkDraftRef.current = initialDraft;
        setRemarkDraft(initialDraft);
        setRemarkSaveState("idle");
        setHitlRequestState("idle");
        setReportLoading(false);
        ...
        if (eventsResult.ok) {
          ...
          if (current) {
            ...
            if (remarkDraftRef.current === withoutEvents) {
              const persistedDraft = effectiveRemarkText(current, eventsResult.payload.events);
              remarkDraftRef.current = persistedDraft;
              setRemarkDraft(persistedDraft);
            }
          }
        } else {
          setReviewEvents([]);
  ...
  const applySelect = useCallback((index: number, issue: ValidationIssue) => {
    selectedIssueIndexRef.current = index;
    setSelectedIssueIndex(index);
    setSelectedClashIndex(null);
    const nextDraft = effectiveRemarkText(issue, reviewEventsRef.current);
    remarkDraftRef.current = nextDraft;
    setRemarkDraft(nextDraft);
    setRemarkSaveState("idle");
    setHitlRequestState("idle");
    setPendingSelect(null);
    setConflictMessage(null);
  ...
  const reloadEventsKeepDraft = useCallback(async (reportId: string) => {
    try {
      ...
      setReviewEvents(payload.events);
      setReviewEventsError(null);
    } catch (error: unknown) {
      if (selectedReportRef.current?.report_id !== reportId) {
        return;
    ...
    async (issue: ValidationIssue | null): Promise<boolean> => {
      ...
      try {
        ...
        if (error instanceof ApiHttpError && error.status === 409) {
          ...
          await reloadEventsKeepDraft(reportId);
        }
        return false;
      } finally {
  ...
  const decideRemark = useCallback(
    async (eventType: "accepted" | "rejected", issue: ValidationIssue | null) => {
      ...
      const previous = latestHitlState(reviewEventsRef.current, issue);
      if (previous !== null && !canStartDecision(previous)) {
        setHitlRequestState("failed");
        return;
      }
      hitlBusyRef.current = true;
      ...
      try {
        await postHitlEvent(issue, eventType, encodeFinalRemarkNote(finalDraft), reportId);
        if (!stillOnFinding(reportId, issue)) {
          return;
        }
        setHitlRequestState("idle");
  ...
  const openRemark = useCallback(
    async (issue: ValidationIssue | null) => {
      ...
      setHitlRequestState("saving");
      try {
        await postHitlEvent(issue, "opened", "", reportId);
        if (!stillOnFinding(reportId, issue)) {
          return;
        }
        setHitlRequestState("idle");
  ...
  const discardRemarkDraft = useCallback(() => {
    const current = selectedReportRef.current?.issues[selectedIssueIndexRef.current];
    const restoredDraft = current ? effectiveRemarkText(current, reviewEventsRef.current) : "";
    remarkDraftRef.current = restoredDraft;
    setRemarkDraft(restoredDraft);
    setRemarkSaveState("idle");
    setHitlRequestState("idle");
    setConflictMessage(null);
    setPendingSelect(null);
  ...
  const persistedHitlState = selectedIssue ? latestHitlState(reviewEvents, selectedIssue) : null;
  const hitlDecisionState: HitlDecisionState =
    hitlRequestState === "saving"
      ? "saving"
      : hitlRequestState === "failed"
        ? "failed"
        : persistedHitlState === "accepted"
          ? "accepted"
