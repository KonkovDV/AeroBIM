...
> **КТ#2 (20.08.2026), Техлаб Москва, задача 07, заказчик — ГК «Самолёт».** Пакет подачи: [`submission/README.md`](submission/README.md). Мы на стадии *доработки*. Одна команда показывает fail-closed доказательную находку на учебном комплекте. Валидация эффективности и внедрение ещё не начались. Checkpoint `NO_GO`, пока нет корпуса Самолёта, двух разметчиков, подписанного профиля приёмки и подтверждения импорта в СОД.

<p align="center">
  <img src="docs/evidence/drawing-overlay-smoke-2026-08/overlay-wall-thickness.png" alt="Оверлей на фикстуре: толщина стены WALL-01 150 мм обведена на листе PDF" width="720">
</p>

<p align="center"><sub>Детерминированная рамка на <em>учебном</em> листе (толщина стены 150 мм / WALL-01). Это не компьютерное зрение, не чертёж заказчика и не точность продукта.</sub></p>
...
| Запрос ТЗ / задачи 07 | Что реально делает клон |
|---|---|
| Приём 2D + BIM + тексты | IFC 2x3 / 4 / 4x3, IDS 1.0, PDF вектор/растр, текст спецификации |
| Сверка модели, чертежей, правил | Детерминированные IFC + IDS + междокументное сравнение (допуск ISO 12006-3) |
...
| Эксперт остаётся ответственным | `summary.passed` — это Shared-gate. LLM/VLM его не пишут ([ADR-001](docs/architecture/ADR-001-verdict-ownership-2026.md)) |
...
| | |
|---|---|
| **Работает на этом клоне** | Учебные комплекты, fail-closed IDS, live CLI, CI, оверлей, структурный BCF |
| **Ждёт заказчика** | Корпус RT-001 · подписанный профиль Самолёта RT-002 · федеративный MEP RT-003 · импорт BCF в их СОД |
...
Оба демо заканчиваются с `summary.passed=false`, и это ожидаемый результат: в учебном комплекте (fixture) заложены дефекты. Это не данные заказчика, и полученные на них числа не являются точностью продукта. Локальный счётчик `pytest` — не CI pin в runtime baseline ниже.
...
Только маршрутизация: ни жюри, ни менторы, ни фонды продукт здесь не рекомендуют.

| Читатель | С чего начать |
|---|---|
| Жюри Техлаба / МИК | Формула выше → [`submission/README.md`](submission/README.md) → команда только что выше |
| Самолёт (ТЗ) | Карта покрытия [`submission/TZ_REQUIREMENTS_COVERAGE_2026_08.md`](submission/TZ_REQUIREMENTS_COVERAGE_2026_08.md) · запрос [`docs/partners/SAMOLET_KT2_ASK_2026_08_15.md`](docs/partners/SAMOLET_KT2_ASK_2026_08_15.md) |
...
| **RT-002** | Нет профиля приёмки, подписанного Самолётом | Официальные IDS Мособлгосэкспертизы уже лежат в `samples/` |
...
Экспорт BCF ZIP — структурный T1 ([`audit/evidence/bcf-structural-handoff-2026-07-25.json`](audit/evidence/bcf-structural-handoff-2026-07-25.json)). Импорт в независимую СОД — **NOT_VERIFIED**. Native DWG отсутствует (fail-closed). Независимая корректность расчётов не реализована: источники сверяются, а не пересчитываются.
...
|---|---|---|
| IDS 1.0 как машинный информационный контракт | IfcTester, fail-closed при версии/загрузке | Хеш комплекта заказчика + подписанный профиль (RT-002) |
| Dual-rater precision до публикации точности | Планировщик Уилсона и harness κ/α есть; κ без меток заказчика нет | Корпус заказчика + два разметчика (RT-001) |
...
| FAIR research software | `CITATION.cff`, реестр лицензий, воспроизводимые команды | Fixture F1 ≠ точность продукта |
...
<details>
<summary>Эндпоинты (локально <code>python -m aerobim.main</code>)</summary>
