...
С 2 апреля 2026 года ЦИМ АГР в IFC обязателен к подаче в Москве: постановление Правительства Москвы № 17-ПП от 16 января 2026 задаёт этот срок; совместное распоряжение ДИТ и ДГП № ДГП-Р-1/26/64-16-6/26 от той же даты утверждает требования к материалам IFC. С 18 августа 2026 совместное распоряжение ДГП/ДИТ № ДГП-Р-56/26/64-16-473/26 уточняет требования к трёхмерным моделям в информационных системах Москвы (страница ДГП на mos.ru). Это **городские правила подачи**, не подписанный профиль приёмки Самолёта и не заявление о точности продукта. Городской мэппинг/выгрузка АГР IFC опубликован под Revit (stroimprosto.mos.ru); AeroBIM нативный RVT не принимает. Для застройщика с пайплайном на RVT качество IFC-выгрузки — регуляторный риск подачи; AeroBIM стоит на шве проверки комплекта до подачи и не конкурирует ни с проверщиком модели (Tangl), ни с СОД (10D).

На входе — модели IFC, наборы правил IDS, листы и тексты спецификаций. На выходе — находки, которые можно провести до листа и до GUID: HTML, JSON, PDF (черновик покрытия) и BCF. Решение остаётся за экспертом. AeroBIM — не среда общих данных, не просмотрщик модели и не замена специалисту.

> **Окно КТ#3 (3–21.09.2026), фича-фриз 18.09, пакет сдачи 19–21.09, Техлаб Москва, задача по автоматизированной верификации проектной и рабочей документации, заказчик — ГК «Самолёт».** Исторический пакет подачи КТ#2 (20.08): [`submission/README.md`](submission/README.md).
>
...
AeroBIM поднимает такой класс находок с прослеживаемостью до листа и до GUID, оставляет вердикт эксперту и никогда не подписывает переход Shared → Published по ISO 19650. Проверщики модели вроде Tangl работают на модели; СОД вроде 10D — на наличии, версиях и маршрутах. AeroBIM работает на **шве между файлами**. Этот репозиторий — не коннектор Tangl и не СОД. Участие в Техлабе — статус программы, а не измеренный результат на комплекте Самолёта.
...
| Подсветка и замечание | Оверлей 2D, оболочка ревью 3D, шаблоны RU/EN, HITL-правка экспертом |
| Отчёт для координации | HTML + JSON + PDF (черновик карты покрытия) + структурный архив BCF 2.1 / 3.0 |
...
| | |
|---|---|
| **Работает на этом клоне** | Учебные комплекты, проверка IDS с отказом при пропуске, живой CLI, CI, оверлей, структурный BCF, оболочка ревью (один экран: находки → замечание → лист/3D → BCF) |
...
# 3. КТ#3 одной командой: живой гейт на фикстуре + пакет + шесть задач трекера (GO; customer_go false)
...
# опциональных publication-denylist секретов GitHub — skip. Локальный счётчик — не CI pin ниже.
python -m aerobim.main   # → http://127.0.0.1:8080/health

# 4. Оболочка ревью (отдельное хранилище; не CLI жюри)
...
Дополнительные наборы: `.[clash]` — геометрические коллизии, `.[docling]` — разбор нетекстовых документов, `.[enterprise]` — адаптеры S3 и Postgres, `.[pdf-agpl]` — устаревшие инструменты на PyMuPDF (для трёх демо-команд выше не нужны; без extra эти тесты skip). Оболочка ревью (одна команда): `python -m aerobim.tools.run_review_stand` из `backend/`, или `python scripts/run_review_shell.py` из корня репозитория, или `npm start` из `frontend/`. Это поднимает API и Vite вместе (`npm ci`, если Vite ещё не ставили). Трек сидящего члена жюри — CLI выше; оболочка — не СОД.

## Оболочка ревью

Браузерное рабочее место — оболочка разбора **сохранённых отчётов**, не СОД и не замена эксперту. UI не пишет `summary.passed` ([ADR-001](docs/architecture/ADR-001-verdict-ownership-2026.md)).

- **Один клик репетиции.** Только development: `POST /v1/demo/seed-fixture` (в OpenAPI не публикуется; git-стены+IDS, не пакет заказчика) или успешный прогон открывает три панели эксперта: находки, замечание (пункт ИТЗ/СТО/СП; этаж/ось или «нет в индексе»), лист и 3D, BCF на той же полосе. Вкладка «Экспорт» не обязательна. PDF остаётся (`GET .../export/pdf` — черновик покрытия). Эндпоинта XLSX нет.
- **Честность.** Баннер возможностей по-русски: тишина ≠ успех; коллизии сетей без IFC сетей названы невыполненными. По умолчанию `GET /v1/auth/bff` = 501. Лабораторный `200 LAB` — не SSO заказчика. HITL: expert/reviewer; `user`/`viewer` в лаборатории → 403.
- **Пределы.** Нативные RVT/NWD/DWG — отказ до загрузки. WASM-вьюер 256 МиБ; разбор на диске на жёстком профиле до 1,5 ГБ через RocksDB. SSE нет, задания опрашиваются.

## Кому эта страница

Жюри Техлаба и МИК: формула выше → [`submission/README.md`](submission/README.md) → CLI выше. Трек сидящего члена жюри — этот CLI. Браузерная оболочка ревью — необязательный UI на этом клоне, не СОД. Это пакет доработки, не акт Checkpoint.
