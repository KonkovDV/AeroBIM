...
Программа Техлаба Москва, задача по автоматизированной верификации проектной и рабочей документации (заказчик канала в публичном дереве не называется). Статус программы — не измеренный результат на комплекте заказчика канала. Индекс пакета: [`submission/README.md`](submission/README.md).
...
| BCF → СОД | Структурный ZIP (T1) | Журнал импорта T2 в СОД заказчика канала |
...
- Честность доступности проверок; разграничение доступа к артефактам на профилях `customer_pilot` / `production` (в development по умолчанию выключено); выгрузка HTML/JSON; PDF — черновик покрытия; структурный архив BCF 2.1 / 3.0
...
**48 Protocol ports** связаны с **76 adapter modules** через **63 DI tokens** в `bootstrap_container()`. Это живой инвентарь: он пересобирается в [`docs/evidence/runtime-baseline-latest.json`](docs/evidence/runtime-baseline-latest.json) и сверяется в CI против обоих README, поэтому руками эти числа не правятся.
...
submission/   Индекс пакета подачи для жюри Техлаба
```
...
tests_passed: backend=3197, frontend=387; commit 8f0a462df68f; see docs/evidence/runtime-baseline-latest.json · src ~103559 LOC; tests ~67043 LOC; extraction macro_f1=0.8600000000000001 (fixture corpus; not product accuracy)
