...
Оба демо и команда КТ#3 заканчиваются с `summary.passed=false`, и это ожидаемый результат: в учебном комплекте заложены дефекты. Это не данные заказчика, и полученные на них числа не являются точностью продукта. Локальный счётчик `pytest` — не CI pin в runtime baseline ниже. Красный бейдж `NO_GO` — это подпись заказчика, не «ядро не запускается».

Дополнительные наборы: `.[clash]` — геометрические коллизии, `.[docling]` — разбор нетекстовых документов, `.[enterprise]` — адаптеры S3 и Postgres, `.[pdf-agpl]` — устаревшие инструменты на PyMuPDF (для трёх демо-команд выше не нужны; без extra эти тесты skip). Оболочка ревью: `cd frontend && npm ci && npm run dev`.
