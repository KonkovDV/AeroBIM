...
> **Пакет подачи КТ#2 (20.08.2026).** Пять полей формы, покрытие ТЗ и граница заявлений: [`submission/README.md`](submission/README.md). Checkpoint `NO_GO`, пока нет evidence заказчика (RT-001 / RT-002 / RT-003).

**AeroBIM проверяет комплект проектной документации так, как к нему подходит эксперт: модель — к правилам, чертежи — к модели, тексты — к тому и другому.**
...
Именно с этим классом задач работает AeroBIM. Система поднимает находку с прослеживаемостью до листа и до GUID, оставляет вердикт эксперту и никогда не подписывает переход Shared → Published по ISO 19650.

Проект участвует в программе «Техлаб Москва» (задача 07, заказчик — ГК «Самолёт»). Это статус участия, а не утверждение, что эффективность измерена на комплекте заказчика.

## Что происходит за один прогон

Комплект на входе. Детерминированные проверки. Один сводный отчёт.

1. **Модель.** Свойства и величины проверяются через IfcOpenShell. IFC2x3 (ISO 16739:2005), IFC4 ADD2 (ISO 16739-1:2018) и IFC4x3 (ISO 16739-1:2024) идут через одно ядро. Расхождение имён наборов свойств между релизами выдаётся как `ValidationIssue`, а не как молчаливый пропуск. Правила по функциям: [`docs/ifc-compatibility-matrix.md`](docs/ifc-compatibility-matrix.md).
2. **Правила.** IDS 1.0 проверяется через IfcTester. Официальные наборы Мособлгосэкспертизы лежат в `samples/`. Запрошенный набор, который не загрузился, роняет проверку — он не может выглядеть как чистый проход.
3. **Остальные документы.** Модель сопоставляется с пометками на чертеже, спецификациями и расчётными текстами, с полем допуска по ISO 12006-3 и с русскими/европейскими группированными числами. Источники сверяются; ничего не пересчитывается. Независимая корректность расчётов не реализована.
...
- **Одинаковый вход — одинаковый `summary.passed`.** Технический флаг собирается из детерминированных ошибок и таблицы доступности проверок ([ADR-001](docs/architecture/ADR-001-verdict-ownership-2026.md)). Советующий текст LLM/VLM, если он вообще включён, только черновит формулировку замечания и никогда не пишет `summary.passed`. На профилях заказчика внешние вызовы советующего контура запрещены.
...
`summary.passed` — это прохождение настроенного Shared-gate. Это не контрактная «пригодность документации» и не разрешение строить. Архитектура: [`docs/architecture/TARGET_HYBRID_ARCHITECTURE_TZ_2026.md`](docs/architecture/TARGET_HYBRID_ARCHITECTURE_TZ_2026.md).
...
Оба демо заканчиваются с `summary.passed=false`, и это ожидаемый результат: в учебном комплекте (fixture) заложены дефекты. Это не данные заказчика, и полученные на них числа не являются точностью продукта.
...
Проверка комплекта до стройки — государственная экспертиза, ГИП, контроль качества документации — на стыке модели, чертежей и требований. Не замена среде общих данных и не полевой журнал дефектов.

С 1 апреля 2026 года ГОСТ Р 21.101-2026 п. 8.2.4 требует устойчивый GUID у каждого электронного документа проектной документации, чтобы отслеживать изменения версий. AeroBIM с первого дня ведёт находки к устойчивому идентификатору элемента модели / источника. Это совпадение механизма, а не заявление о полном соответствии стандарту.

## Checkpoint: `NO_GO`

Это внутренняя оценка готовности к *подписанию у заказчика*, а не утверждение «система не работает». Код и учебные комплекты работают. Открытыми остаются три блокера, и ни один из них не снимается кодом:

- **RT-001** — нет корпуса российской проектной документации с заключениями экспертизы. Открытые AEC-Bench, IFC-Bench и GNI существуют, но это другой набор данных.
...
Реестр блокеров: [`audit/reports/CRITICAL_BLOCKERS.md`](audit/reports/CRITICAL_BLOCKERS.md) · граница утверждений: [`docs/pilot-claim-boundary-2026.md`](docs/pilot-claim-boundary-2026.md) · инженерный статус: [`docs/ENGINEERING_STATUS_2026_08.md`](docs/ENGINEERING_STATUS_2026_08.md).

Не заявляется до доказательств: accuracy >90%, DWG-ready, MEP delivered, CDE-ready BCF, human-level CV, production-ready. Зелёный прогон на учебных комплектах — не подписание. Разбор файлов DWG без конвертации отсутствует (fail-closed); PDF или IFC можно принять как производный вход с прослеживаемостью — это не заявляется как DWG-ready. MEP system clash delivered не заявляется (RT-003, всегда `geometry_verified=False`). Экспорт BCF ZIP подтверждён двумя независимыми потребителями ([`audit/evidence/bcf-structural-handoff-2026-07-25.json`](audit/evidence/bcf-structural-handoff-2026-07-25.json)); импорт в независимую среду общих данных остаётся **NOT_VERIFIED** ([`audit/evidence/cde-import-proof/STATUS.json`](audit/evidence/cde-import-proof/STATUS.json)). До появления такого доказательства формулировки «BCF ready for CDE» и «CDE interoperable» запрещены. Лестница доказательств: [`docs/architecture/BCF_EVIDENCE_LADDER_T0_T4_2026_07.md`](docs/architecture/BCF_EVIDENCE_LADDER_T0_T4_2026_07.md).
...
- Проверка свойств и величин IFC; IDS 1.0, fail-closed если набор правил не загрузился
...
- Допуски ISO 12006-3; извлечение требований из текста по детерминированным шаблонам (ни одна модель ничего не подписывает)
- Честность доступности проверок; разграничение доступа к артефактам; выгрузка HTML/JSON; структурный архив BCF 2.1 / 3.0
- Детерминированная работа с PDF (pypdfium2 + pdfminer)
- Просмотр IFC в браузере и оверлей 2D; автономная сборка Docker (`closed-contour --smoke`; без Docker — вне scope)
- Паки нормативных правил (применимость и журнал эксперта; пак-фикстура не является подписанным профилем заказчика) и по желанию инвентарь комплектности (не нормативный вывод)
...
- Браузерная сессия OIDC: не реализовано (по умолчанию 501); лабораторный контур cookie не является production SSO
...
tests_passed: backend=2455, frontend=56; commit acac02bd9d2d; see docs/evidence/runtime-baseline-latest.json · src ~77804 LOC; tests ~51152 LOC; extraction macro_f1=0.8600000000000001 (fixture corpus; not product accuracy)
