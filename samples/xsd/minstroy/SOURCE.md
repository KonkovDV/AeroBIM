...
**Retrieved:** 2026-08-14 (evening re-probe of subsections `p9_4/` and `p12_2/`)  
**Claim:** published XSD 1.1 for *intake* pre-check (well-formed XML, declared root, XSD where XMLSchema11 can load the file, including a load-time strip of duplicate `xml:id` on `xs:documentation`). Not GrK art. 49 expertise. Not a remark corpus. Not УКЭП. Does **not** close RT-001.
...
All intake XSDs declare `vc:minVersion="1.1"`.
