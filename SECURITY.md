...
## Deployment Hardening (Wave 0–2 + RT-POST 2026-07-19 + RTATOM A1/A2 2026-07-20)
...
- Tenant-scoped filesystem artifact reads assert `tenants/{encoded}/` prefix; `safe_storage_token` encodes `/\_:` to avoid prefix collisions.
...
- Storage path resolution rejects symlinks and path escapes under `AEROBIM_STORAGE_DIR`; ZIP members with `..` or absolute paths are rejected (including BCF consumers via `inspect_zip_bytes`).
