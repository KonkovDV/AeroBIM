...
`pytest tests -q` after `pip install -e ".[dev,raster]"` is the jury clone contract: **0 failed**. Optional `pdf-agpl` and publication-denylist GitHub secrets are CI-only; those tests skip without them. Do not treat a local count as [`runtime-baseline-latest.json`](../../docs/evidence/runtime-baseline-latest.json).
...
[`../../samples/`](../../samples/) — учебные комплекты IFC, IDS, чертежей и спецификаций. Не выгрузка Renga и не комплект заказчика. Приложения ТЗ: [`../../samples/tz-appendix/README.md`](../../samples/tz-appendix/README.md).
