...
class InterpretationUseLedgerTests(unittest.TestCase):
    def test_ledger_is_internally_consistent(self) -> None:
        ...
        self.assertTrue({"channel", "tracker", "techlab", "mik", "industry"}.issubset(sources))
    ...
    def test_payload_stays_no_go(self) -> None:
        ...
        self.assertIn("CH-11", ids)
        self.assertIn("IND-27", ids)
        self.assertIn("IND-28", ids)
        self.assertIn("CH-12", ids)
        self.assertIn("IND-29", ids)
        self.assertIn("PLAN-06", ids)
        self.assertIn("TL-17", ids)
        self.assertIn("CH-10", ids)
        self.assertIn("PLAN-00", ids)
        self.assertIn("PLAN-05", ids)
        self.assertIn("SIG-01", ids)
        self.assertIn("SIG-02", ids)
        self.assertIn("JURY-01", ids)
        self.assertIn("FMT-01", ids)
        self.assertIn("SPG-01", ids)
        self.assertIn("UI-01", ids)
        ui01 = next(row for row in payload["rows"] if row["row_id"] == "UI-01")
        self.assertIn("Jury laptop stays CLI", ui01["licensed_inference"])
        self.assertNotIn("seed as customer pack", ui01["licensed_inference"])
        trk01 = next(row for row in payload["rows"] if row["row_id"] == "TRK-01")
        self.assertIn("run_kt3_jury", trk01["licensed_inference"])
        self.assertIn("KT3_TRACKER_SIX_TASKS_2026_08.md", trk01["evidence"])
