...
Windows cannot `pip install --require-hashes -r requirements-dev-lock.txt`: the Linux lock includes `uvloop`, which refuses to build on Windows. Local Windows clone path remains `.\.venv\Scripts\python.exe -m pip install -e ".[dev,raster]"` (README [Try it](../README.md#try-it)). Optional `pdf-agpl` is not required for the KT#2 overlay (pypdfium2). Hashed lock is the CI/Linux/Docker install.
