...
def _install_urllib3_dial_pin() -> None:
    ...
    if urllib3_http is not None:
        ...
        if bound is not None and not getattr(bound, "_aerobim_outbound_pin", False):
            ...
            urllib3_http.create_connection = _pinned_urllib3_http_create_connection  # type: ignore[attr-defined]

        http_conn = getattr(urllib3_http, "HTTPConnection", None)
        if http_conn is not None:
            orig_new = getattr(http_conn, "_new_conn", None)
            if callable(orig_new) and not getattr(orig_new, "_aerobim_outbound_pin", False):
                ...
                http_conn._new_conn = _pinned_urllib3_new_conn
