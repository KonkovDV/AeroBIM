...
class AIProvenanceRecord:
    ...
    temperature: Optional[float]  # Relevant sampling config
    input_hashes: list[str]    # sha256 of each input passed to model
    output_hash: str           # sha256 of raw model output
    timestamp: datetime = field(default_factory=lambda: datetime.now(tz=timezone.utc))
    tenant_id: str = ""
    project_id: str = ""
    finding_id: Optional[str] = None  # If linked to a finding
    evidence_id: Optional[str] = None  # If linked to evidence
...
class AIEvaluationMetrics:
    ...
    factuality_rate: Optional[float] = None       # 0-1
    grounding_rate: Optional[float] = None        # Citations valid
    abstention_rate: Optional[float] = None       # Model said "I don't know"
    hallucination_rate: Optional[float] = None    # Unsupported evidence refs
    unsupported_claim_rate: Optional[float] = None  # Normative statements w/o basis
    precision: Optional[float] = None
    recall: Optional[float] = None
    f1: Optional[float] = None
    human_agreement_rate: Optional[float] = None  # Double-annotated subset
    notes: str = ""
