...
def _load_drawing_sources(request_data: dict[str, object], repo_root_path: Path) -> tuple[DrawingSource, ...]:
    drawing_sources: list[DrawingSource] = []
    for item in request_data.get("drawings", []):
        drawing_data = dict(item)
        resolved_path = _resolve_repo_path(str(drawing_data["path"]), repo_root_path)
        drawing_format = str(drawing_data.get("format") or "text")
        drawing_text = ""
        if resolved_path.suffix.lower() in {".txt", ".json", ".md"} or drawing_format.lower() in {"text", "json"}:
            drawing_text = _read_text(resolved_path)
        drawing_sources.append(
            DrawingSource(
                text=drawing_text,
                path=resolved_path,
                sheet_id=str(drawing_data.get("sheet_id")) if drawing_data.get("sheet_id") else None,
...
def load_benchmark_pack(manifest_path: Path, repo_root_path: Path | None = None) -> BenchmarkPack:
    ...
    request = ValidationRequest(
        ...
        ids_path=_resolve_repo_path(str(ids_path_raw), resolved_repo_root) if ids_path_raw else None,
        origin="benchmark",
...
def summarize_benchmark_runs(measured_runs: list[dict[str, object]]) -> dict[str, float]:
    if not measured_runs:
        raise ValueError("Benchmark summary requires at least one measured run")
    elapsed_values = [float(run["elapsed_ms"]) for run in measured_runs]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a fixture-backed benchmark rail for analyze/project-package")
    parser.add_argument("--pack", type=Path, default=default_pack_path(), help="Path to a benchmark pack manifest")
    parser.add_argument("--iterations", type=int, default=3, help="Number of measured benchmark iterations")
    parser.add_argument("--warmup-iterations", type=int, default=1, help="Number of unmeasured warmup iterations")
    parser.add_argument("--storage-dir", type=Path, default=None, help="Optional storage directory for persisted benchmark reports")
