    ...
    (bundle_dir / _INSTALL_SH).write_text(
        "\n".join(
            [
                ...
                '  echo "Docker is required for the closed-contour install track. Not a pip wheelhouse; not requirements-win-lock.txt." >&2',
                "  exit 1",
                "fi",
                'if [ -z "${TOKEN}" ]; then',
    ...
    (bundle_dir / _INSTALL_PS1).write_text(
        "\n".join(
            [
                ...
                '  throw "Docker is required for the closed-contour install track. Not a pip wheelhouse; not requirements-win-lock.txt."',
                "}",
                "",
