# TZ Runtime Matrix — the appointing party Task 07
...
| 26 | SLA ≤30 мин | measure_package_sla (gate=**p95**) | CLI | `test_measure_package_sla.py`; evidence `sla-fixture-p95-2026-08-04.json` | fixture | FIXTURE_ONLY; customer НЕ ДОКАЗАНО | HIGH |
...
| 32 | Приём файлов: офис ≤500 МБ, модели ≤1,5 ГБ (п. 1.1.4) | `upload_limits` + RocksDB over SPF | ingest + disk analyze | `test_customer_channel_answers_2026_08_25.py` `test_ifc_size_policy.py` | none | PARTIAL^[fn32] | CRITICAL |
...
^[fn32]: Ответы 1.1.4: офис до 500 МБ (десятичные), модели до 1,5 ГБ. Под `customer_pilot`/`production` это **ingest и disk-analyze**. SPF `ifcopenshell.open(.ifc)` и WASM остаются **256 МиБ**. Файлы больше SPF-капа и до 1,5 ГБ идут в IfcOpenShell RocksDB (`wired_over_spf_cap`), не в SPF RAM. HTTP 413 (`IFC exceeds analyze size limit`) — **свыше 1,5 ГБ**. Convert/open failure — 503 `IFC disk backend unavailable`. bSI Validation Service — **256 MB** несжатого `.ifc` (не MiB). SPF в ifcopenshell — порядка 8–10× диск в RAM ([#7116](https://github.com/IfcOpenShell/IfcOpenShell/issues/7116)); 1,5 ГБ SPF ≈ ~15 ГиБ RSS литература, поэтому SPF default не поднят. 1,5 ГБ ≈ практический потолок toolkit экспорта Revit, не обещание SPF-`open()`. RSS на файле заказчика не замерен (OA-16). Development HTTP без the appointing party caps держит `max_model_bytes=256 МиБ`. SSOT: [`IFC_ANALYZE_VS_INGEST_CAP_2026_08.md`](../../docs/quality/IFC_ANALYZE_VS_INGEST_CAP_2026_08.md).
