...
**Publishable test counts** are only in [`docs/evidence/runtime-baseline-latest.json`](../../docs/evidence/runtime-baseline-latest.json) (`attested_by=ci`: `commit_sha`, `tests_passed`, `tests_collected`). Do **not** copy those integers into this file. **Operational freeze SHA (historical, 2026-07-21):** `f2615e7` (eng F–L: precision gates, SLA claim gate, BCF ladder, revision compare, threat model, open-core ADR). Do **not** treat this SHA as HEAD or as the publishable metrics pin. Prior same-day pin `019962141606` is **historical**. A clean-clone `pytest` on HEAD (README extras, no kitchen secrets) is **not** this pin; HEAD may be several commits ahead. Jury contract on the documented extra is **0 failed** (skips allowed).  
...
- **Product HOLD — undifferentiated RT-002 still OPEN as a customer blocker** until signed Samolet profile. Public MOEXP / SPb CGE / AGR IDS close **RT-002a** (measurement ruler), not RT-002b. Never write undifferentiated «RT-002 CLOSED».
...
- **Product HOLD — undifferentiated RT-003 still OPEN** until customer-side IFC export of the federated (NWD) set + signed scope + verified geometry. **Addendum 2026-09-04:** planted in-repo IfcClash closes **RT-003a** (geometric rehearsal). Evening: three NWD federations close the NWD-carrier volume; native unread; 0 duct/pipe/cable IFC. `mep_system_clash` stays NOT_VERIFIED (**RT-003b**). Pipe vs wall on a fixture is geometry, not an IfcSystem graph. Inventory, planted clash, NWD count, and public duplex clash ≠ MEP delivered. Never write undifferentiated «RT-003 CLOSED».  
- **Addendum 28.08.2026 (критерий ТЗ неизмерим на поставленном пакете):** TZ ставит порог «коллизии > 90%», но по пину инвентаря `PUBLIC_REHEARSAL.federated_mep_ifc_present=false` и `rd_ifc_present=false` — федеративного набора и рабочей IFC в пакете нет, значит критерий на этих данных не измеряется математически, а не «не измерялся пока». Речь: «коллизии проверены на синтетических федеративных моделях; сводная модель у заказчика существует в NWD — запрошена её выгрузка в IFC по одному корпусу штатным пакетным экспортом». Формулировка «закрыли коллизии на данных заказчика» ложна автоматически — см. RT-CLASH-MEASURE в [`docs/quality/TZ_LIVE_TREE_TRIAGE_2026_08_27.md`](../../docs/quality/TZ_LIVE_TREE_TRIAGE_2026_08_27.md). Уточнение тем же вечером (п. 1.1.5 ответов): запрос меняется с «дайте федеративный IFC» на «прогоните по одному корпусу штатный пакетный экспорт NWD→IFC, который уже куплен». До выгрузки неизмеримость остаётся в силе.  
...
- **СТАТУС: ЗАКРЫТО (RT-E remediation 2026-07-17)** — real UC path, advisory ON/OFF; deterministic findings + `summary.passed` equality. Does **not** close RT-001/002/003 or flip `customer_go`. Checkpoint stays **GO** (`regulatory_measurement_mvp`).  
...
**Checkpoint rule:** undifferentiated «RT-001/002/003 CLOSED» or `customer_go=true` without residual volumes ⇒ honesty fail. Product Checkpoint stays **GO** (`regulatory_measurement_mvp`); `customer_go` stays false. Historical July wording «automatic NO_GO» is not the live product flag.
