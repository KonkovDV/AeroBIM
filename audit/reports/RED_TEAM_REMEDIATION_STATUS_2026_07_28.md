...
- **Redis `SET NX` (§4.3):** дефолтный/тестовый стор — `InMemory` (Lock + `can_transition` + idempotency-дедуп). Распределённого Redis-lock `SET NX` в коде **не найдено**; если Redis-бэкенд стора существует — отдельная верификация (не выдаю InMemory-lock за распределённый).
